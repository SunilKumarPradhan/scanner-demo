...
def greet():
    ...
    # SECURITY FIX: Don't construct HTML from user input
...
def ping():
    ...
    # SECURITY FIX: Don't construct OS commands from user input
    import shlex
...
def fetch_url():
    ...
    from urllib.parse import urlparse
    parsed = urlparse(url)
    if parsed.scheme not in ['http', 'https'] or parsed.netloc not in ['api.example.com', 'trusted.example.com']:
