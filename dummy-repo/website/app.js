...
function loadContentFromHash() {
    ...
    if (hash) {
        // SECURITY FIX: Removed eval - use safe alternative
        const content = hash;
...
// SECURITY FIX: Verify origin before processing messages
globalThis.addEventListener('message', function(event) {
    if (event.origin !== globalThis.location.origin) {
        return;
    }
    const data = event.data;
    // SECURITY FIX: Removed eval
});
...
// SECURITY FIX: Removed document.write usage
